...
退出码：`0` 表示成功、仅有 warning 或用户取消；`1` 表示检查发现规范错误，或规范化后仍有 manual 项；`2` 表示参数、编码、I/O、备份或替换失败。首版只处理单个有效 UTF-8 `.ass` 文件，不提供 JSON、批量目录、stdin、SSA/ASS2、32 位目标、自动更新、签名、公证或包管理器分发。
